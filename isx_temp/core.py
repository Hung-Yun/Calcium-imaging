...
class Time(object):
    ...
    @classmethod
    def _from_secs_since_epoch(cls, secs_since_epoch):
        self = cls()
...
class Timing(object):
    ...
    def __init__(self, num_samples=0, period=Duration.from_msecs(50), start=Time(), dropped=[], cropped=[]):
        """
    ...
    def __eq__(self, other):
        return ((self.start == other.start) and
                ...
                (self.cropped == self.cropped))

    def __str__(self):
        return 'Timing(num_samples={}, period={}, start={}, dropped={}, cropped={})'.format(
                self.num_samples, self.period, self.start, self.dropped, self.cropped)
