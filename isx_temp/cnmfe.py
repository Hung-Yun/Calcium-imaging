...
from .io import Movie, CellSet, EventSet, export_movie_to_tiff
...
def run_cnmfe(input_movie_files, output_cell_set_files, output_events_files,
    ...
    cnmfe_wrapper_commands = ['python', '-m', 'isx_onacide_wrapper.runner', '--input_files']
    for tiff_file in tiff_files:
        cnmfe_wrapper_commands.append(tiff_file)
    cnmfe_wrapper_commands += ['--params_file', param_file, '--output_file', out_file]

    proc = subprocess.run(cnmfe_wrapper_commands)
